...
export class AuthService {
    ...
    async login(user: LoginResponseDto, ip?: string | undefined, userAgent?: string | null) {
        ...
        return {
            ...
            refreshToken,
            tokenType: 'bearer',
            expiresIn: Number.parseInt(this.configService.getOrThrow<string>('JWT_TTL_M'), 10) * 60,
        }
    }
    ...
    async refresh(refreshToken: string) {
        ...
        try {
            const payload = this.jwtService.verify<JwtPayload & { version?: number }>(refreshToken, {
            ...
            const accessToken = this.jwtService.sign(this.createTokenPayload(user, session.id), this.getJwtOptions(
                'JWT_ACCESS_TOKEN_SECRET',
                `${this.configService.getOrThrow<string>('JWT_TTL_M')}m`
            ))
